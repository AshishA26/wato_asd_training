...
class CostmapCore {
  public:
    ...
    void initializeCostmap(double resolution, int width, int height, int8_t default_cell_value);
    ...
    void inflateObstacles(double inflation_radius, int8_t max_cost_inflated_cell, int8_t cost_occupied);
  ...
  private:
    ...
    std::vector<std::vector<int8_t>> occupancy_grid_; // 2D array representing occupancy grid
