...
CostmapCore::CostmapCore(const rclcpp::Logger& logger) : logger_(logger), resolution_(0.0), width_(0), height_(0) {}

void CostmapCore::initializeCostmap(double resolution, int width, int height, int8_t default_cell_value) {
    // Store the costmap parameters
    resolution_ = resolution;
    width_ = width;
    height_ = height;

    // Create a 2D array representing the OccupancyGrid. Each cell corresponds to a grid
    // space in the real world. Initialize all cells to a default value (0 for free space).
    occupancy_grid_.assign(height_, std::vector<int8_t>(width_, default_cell_value));
}
...
void CostmapCore::convertToGrid(double range, double angle, int &x_grid, int &y_grid) {
    ...
    // (origin is assumed to be at the center of the grid)
...
void CostmapCore::markObstacle(int x, int y, int8_t cost_occupied) {
    // Set the cells corresponding to detected obstacle positions to a high cost value
    if (x >= 0 && x < width_ && y >= 0 && y < height_) {
        occupancy_grid_[y][x] = cost_occupied;
    }
}

void CostmapCore::inflateObstacles(double inflation_radius, int8_t max_cost_inflated_cell, int8_t cost_occupied) {
    ...
    for (int y = 0; y < height_; ++y) {
        for (int x = 0; x < width_; ++x) {
            ...
            // - If the distance is within the radius, assign a cost to the surrounding cell 
            //   based on its dist to the obstacle cell (closer cells get higher cost).
            for (int dy = -radius; dy <= radius; ++dy) {
                for (int dx = -radius; dx <= radius; ++dx) {
                    ...
                    if (nx >= 0 && nx < width_ && ny >= 0 && ny < height_) {
                        ...
                        // If cell is within the inflation radius, then calculate the new inflated cost
                        // and assign only if the calculated cost is higher than the cell's current value
                        if (dist <= inflation_radius) {
                            int8_t inflated_cost = static_cast<int8_t>((1 - dist / inflation_radius) * max_cost_inflated_cell);
                            inflated_grid[ny][nx] = std::max(inflated_grid[ny][nx], inflated_cost);
                        }
                    }
                }
            }     
